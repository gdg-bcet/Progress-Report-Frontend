  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        const computedPercent = Math.round((completedCount / (badges.length || 1)) * 100);

        setUserData({
          name: data?.name || data?.displayName || "Unknown User",
          profile: data?.profile || data?.profileUrl || "",
          badges,
          completion_percentage: (typeof data?.completion_percentage === "number")
          ...
          <div className='flex items-center gap-4  '>
            ...
            <div>
              <h3 className="scroll-m-20 text-2xl font-semibold tracking-tight">
                {badges.filter(b => b && b.completed).length} / {badges.length} badges
              </h3>
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 items-center gap-4 pb-6 border-b pt-6 border-t">
            <div className='pr-4 flex justify-center items-center'>
              <AnimatedCircularProgressBar value={(userData.completion_percentage)} className="size-50"gaugePrimaryColor="var(--google-blue)"
          gaugeSecondaryColor="rgba(0, 0, 0, 0.1)" />
            </div>
            <div className='flex flex-col pl-2'>
                <div className='border-b pb-2 font-medium text-lg'>Completion Timeline</div>
                {badges.length > 0 ? (
                  <div className='mt-2'>
                    ...
                    .map((badge, idx) => {
                      const completedAt = badge?.timestamp
                        ? new Date(badge.timestamp).toLocaleDateString('en-US', {
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric',
                          })
                      ...
                      return (
                        <div key={badge?.id ?? idx} className="mb-2">
                          ...
                          <Badge className="bg-white text-blue-800 text-base  mr-2">
                            <FaCheckCircle className="text-green-500 w-10 h-10" />
                  ...
                  <p className="text-sm text-slate-500">No badges completed yet.</p>
                )}
          ...
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {badges.length === 0 && (
              <div className="col-span-2 text-sm text-slate-500">No badges yet.</div>
            )}
            ...
            .map((badge, idx) => (
              <div key={badge?.id ?? idx} className="p-4 border rounded">
                <div className="flex justify-between items-center">
                  <h4 className=" font-medium size-0.7">{badge?.name ?? "Unnamed Badge"}</h4>
                  <div>
